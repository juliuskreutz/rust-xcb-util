...
impl QueryPictFormatsReplyExt for QueryPictFormatsReply {
    fn find_visual_format(&self, visual: xcb::Visualid) -> Option<Pictvisual> {
        let result = unsafe { xcb_render_util_find_visual_format(self.ptr, visual) };
    ...
    fn find_format(&self, mask: Pictformat, formats: &[Pictforminfo]) -> Option<Pictforminfo> {
        let formats: Vec<xcb::ffi::render::xcb_render_pictforminfo_t> =
            formats.into_iter().map(|format| format.base).collect();

        let result = unsafe {
            xcb_render_util_find_format(self.ptr, mask, formats.as_ptr(), formats.len() as _)
        };
    ...
    fn find_standard_format(&self, format: PictStandard) -> Option<Pictforminfo> {
        let result = unsafe { xcb_render_util_find_standard_format(self.ptr, format) };
