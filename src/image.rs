...
impl Image {
    pub fn annotate(&self) {
        unsafe { xcb_image_annotate(self.0) }
    }
    ...
    pub fn data(&self) -> &[u8] {
        unsafe { slice::from_raw_parts((*self.0).data, (*self.0).size as usize) }
    }

    pub fn put(&mut self, x: u32, y: u32, pixel: u32) {
        unsafe { xcb_image_put_pixel(self.0, x, y, pixel) }
    }

    pub fn get(&self, x: u32, y: u32) -> u32 {
        unsafe { xcb_image_get_pixel(self.0, x, y) }
    }
}
...
pub fn create<T: AsRef<[u8]>>(source: T, width: u32, height: u32) -> Image {
    unsafe {
        ...
        Image(xcb_image_create_from_bitmap_data(
            data as *mut _,
            width,
            height,
        ))
...
pub fn is_native(c: &xcb::Connection, image: &Image) -> bool {
    unsafe { xcb_image_native(c.get_raw_conn(), image.0, 0) == image.0 }
}
    ...
    pub fn create(c: &xcb::Connection, depth: u8, width: u16, height: u16) -> Result<Image, ()> {
        unsafe {
            ...
            let image = xcb_image_create(
                ...
                setup.image_byte_order() as u32,
                setup.bitmap_format_bit_order() as u32,
                ptr::null_mut(),
                !0,
                ptr::null_mut(),
            );
